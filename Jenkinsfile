            ...
            steps {
                script {
                    def ExportResponse = bat(
                        script: """
                            curl -H "Content-Type: text/xml" -X POST -H "Authorization: Bearer ${XRAY_TOKEN}"  --data @"output.xml" https://xray.cloud.getxray.app/api/v2/import/execution/robot?projectKey=POEI25-2
